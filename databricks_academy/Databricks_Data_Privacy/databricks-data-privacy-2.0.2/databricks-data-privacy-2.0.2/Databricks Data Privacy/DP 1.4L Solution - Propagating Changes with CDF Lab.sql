...
-- MAGIC
-- MAGIC We'll focus on the **user_lookup** table with the intent to delete a user, check the CDF changes and then propagate such changes into the tables **users** and **user_bins**.
-- MAGIC
...
-- MAGIC - **user_bins**: Table containing user bin assignments.
...
-- MAGIC
-- MAGIC To delete record from the table pick a value from the **device_id** column from above to delete. Then complete the following:
-- MAGIC
...
-- MAGIC %md
-- MAGIC ## H. Review user_looup table History
-- MAGIC
