...
# MAGIC * Generate and trigger a Delta Live Table pipeline that manages both processes
...
# MAGIC %md
# MAGIC
# MAGIC ## B. Generate and Trigger DLT Pipeline
...
# MAGIC
# MAGIC This Delta Live Tables Pipeline is based in two notebooks located in the "Pipeline" folder:
# MAGIC
...
# MAGIC %md
# MAGIC ### B1. Open the DLT Pipeline
# MAGIC
# MAGIC In the left navigation bar, complete the following to open your DLT pipeline:
# MAGIC
# MAGIC 1. Right-click on Pipelines and select *Open in New Tab*.
# MAGIC
# MAGIC 2. Find and select your DLT pipeline named **your_catalog_name_1.2_PII_Data_Security**.
# MAGIC
# MAGIC 3. Leave the DLT pipeline page open and continue to the next steps.
# MAGIC
# MAGIC 4. Once the pipeline completes, here is the graphed execution flow:
# MAGIC
# MAGIC ![PII Data Security DLT Pipeline DAG](./Includes/images/piidata_security_dag.png)
...
# MAGIC
# MAGIC Run the cell below to land more data in the source directory, then navigate to the Delta Live Table UI and manually trigger a pipeline update.
# MAGIC
# MAGIC As we continue through the course, you can return to this notebook and use the method provided below to land new data. Running this entire notebook again will delete the underlying data files for both the source data and your DLT Pipeline and enable you to start over.
