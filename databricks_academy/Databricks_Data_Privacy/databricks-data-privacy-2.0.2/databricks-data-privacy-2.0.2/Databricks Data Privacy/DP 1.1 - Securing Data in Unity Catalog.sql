...
-- MAGIC
-- MAGIC 1. Run the code below to view the dynamically set `DA.catalog_name` course variable in a simple print statement. Remember, the `DA` object is a Databricks Academy variable set dynamically during the classroom setup scripts for the labs.

-- COMMAND ----------

-- MAGIC %python
-- MAGIC
-- MAGIC print(f"SELECT * FROM {DA.catalog_name}.pii_data.customers_gold_view")

-- COMMAND ----------

-- MAGIC %md
-- MAGIC 2. Run the query below to view the results of the **customers_gold_view** you created earlier using the three level naming convention.
...
-- MAGIC r = spark.sql(f'''
-- MAGIC               SELECT * 
-- MAGIC               FROM {DA.catalog_name}.pii_data.customers_gold_view
-- MAGIC ''')
-- MAGIC
-- MAGIC display(r)
...
-- MAGIC %md
-- MAGIC 3. Run the query below using just the view name **customers_gold_view**. Notice that this works as well. 
-- MAGIC
...
-- MAGIC
-- MAGIC 2. Let's create a view to redact the **customer_id** column and hide high loyalty customers, so that only members of `supervisors` group can see it using the `is_account_group_member()` function.
-- MAGIC
...
-- MAGIC %md
-- MAGIC 3. Now create lets the table **customers_silver_with_row_filter_and_column_masks** from the **customers_silver** table and view it's results. 
-- MAGIC
...
-- MAGIC %md
-- MAGIC 4. Now let's assign the `loyalty_filter` function with `WITH ROW FILTER` using an `ALTER TABLE` statement on the table `customers_silver_with_row_filter_and_column_masks` to the **loyalty_segment** column.
-- MAGIC
